...
void compute_Nwiggle(struct mypulsar * pulsar)
{
  int N = pulsar->N;
  int N_m = pulsar->N_m;
  Cmatrix * GNG, *GNG_temp, *Cholesky, *GNG_temp2;
  GNG = new Cmatrix(N_m,N_m);
  GNG_temp = new Cmatrix(N_m,N);
...
void get_G_matrix(Cmatrix * G, pulsar * psr, struct mypulsar mypsr)
{
  int j,k;
  ...
  int ma = ndim;
  Cmatrix * design;
  design = new Cmatrix(ma,mypsr.N);
  Cmatrix * designT;
  designT = new Cmatrix(mypsr.N,ma);
  for (j = 0; j < mypsr.N; j++)
    {
  ...
  Cvector * singulars;
  Cmatrix *U;
  U = new Cmatrix(mypsr.N,mypsr.N);
  singulars = new Cvector(mypsr.N);
  ...
  *Ntot = 0;
  ...
  for (i = 0; i < Nplsr; i++)
      ...
      if (only_res == 0) //do all init
	{
	  ...
	  pulsars[i].toa = new Cvector(&totalsize,pulsars[i].N);
...
//		}
	  ...
	  //if (fscanf(infile,"%d %d %d %d\n",&(pulsars[i].N),&(pulsars[i].N_m),&(pulsars[i].n_be),&(pulsars[i].n_sample)) != 0);
	      pulsars[i].res = new Cvector(&totalsize,pulsars[i].N);
	      pulsars[i].obsfreqs = new Cvector(&totalsize,pulsars[i].N);
      ...
      if (only_res == 0)
	{
	  ...
	  for (j = 0; j < pulsars[i].N; j++)
	    {
	      int inew = 0;
	      while (sorted[inew] != pulsars[i].backends[j])
		inew++;
	      if (verbose == 3)
		printf("%d\t%d\n",inew,pulsars[i].backends[j]);
	      pulsars[i].backends[j] = inew;
	    }
	  ...
	  pulsars[i].G = new Cmatrix(&totalsize,pulsars[i].N,pulsars[i].N_m);
	  

	  *Ndim += pulsars[i].N_m;
	  *Ntot += pulsars[i].N;
	  pulsars[i].CWN = new Cmatrix(&totalsize,pulsars[i].N,pulsars[i].N);
      //pulsars[i].CWN = gsl_matrix_calloc(pulsars[i].N,pulsars[i].N); //initialize with 0s

#ifdef DM
	  pulsars[i].phi_inv = new Cvector(&totalsize,2*NFFT);
	  pulsars[i].F = new Cmatrix(&totalsize,pulsars[i].N,2*NFFT);
	  pulsars[i].GF = new Cmatrix(&totalsize,pulsars[i].N_m,2*NFFT);
#else
	  pulsars[i].phi_inv = new Cvector(&totalsize,NFFT);
	  pulsars[i].F = new Cmatrix(&totalsize,pulsars[i].N,NFFT);
	  pulsars[i].GF = new Cmatrix(&totalsize,pulsars[i].N_m,NFFT);
#endif
	  
	  pulsars[i].GNGinv =  new Cmatrix(&totalsize,pulsars[i].N_m,pulsars[i].N_m);
	  pulsars[i].H = new Cmatrix(&totalsize,pulsars[i].N,2);
	  
	  //	  pulsars[i].C = new Cmatrix(&totalsize,pulsars[i].N_m,pulsars[i].N_m);
	  pulsars[i].Cinv = new Cmatrix(&totalsize,pulsars[i].N_m,pulsars[i].N_m);
	  pulsars[i].L = new Cmatrix(&totalsize,pulsars[i].N_m,pulsars[i].N_m);
	  pulsars[i].GH = new Cmatrix(&totalsize,pulsars[i].N_m,2);
	  
	  pulsars[i].Gres = new Cvector(&totalsize,pulsars[i].N_m);
	} //end if only_res == 0
      //Do this part always
      ...
      if (only_res == 0)
	{
	  ...
	  pulsars[i].sample = new Cmatrix(&totalsize,xdim,N_SAMPLE_MAX);
	  char infilename[120];
...
void compute_C_matrix(struct mypulsar * psr, struct parameters * par)
{
  ...
  Cmatrix * FNF, *FN;
#ifdef DM
  FN = new Cmatrix(2*NFFT,psr->N_m);
  FNF = new Cmatrix(2*NFFT,2*NFFT);
#else
  FN = new Cmatrix(NFFT,psr->N_m);
  FNF = new Cmatrix(NFFT,NFFT);
#endif
  ...
  Cmatrix * FNF_lu;
#ifdef DM
  FNF_lu = new Cmatrix(2*NFFT,2*NFFT);
#else
  FNF_lu = new Cmatrix(NFFT,NFFT);
#endif

  *FNF += psr->phi_inv; //equivalent to former my_matrix_add_diagonal
  ...
  Cmatrix *FNFFN;
#ifdef DM
  FNFFN = new Cmatrix(2*NFFT,psr->N_m);
#else
  FNFFN = new Cmatrix(NFFT,psr->N_m);
#endif
...
void add_signal(struct mypulsar *psr, pulsar * t_psr, struct parameters params, struct source source_pars, int Nplsr)
{
  int p;
  for (p = 0; p < Nplsr; p++)
    {

  longdouble theta_a = 0.5*PI - psr[p].dec;
  longdouble phi_a = psr[p].raj;
  longdouble L_a = psr[p].dist; //distance to the pulsar
  
  longdouble theta_s = source_pars.theta_s;
  longdouble phi_s = source_pars.phi_s;
  longdouble Mc = source_pars.Mc;
  longdouble fr = source_pars.fr;
  //longdouble fr = params.omega/(2.0*PI);
  
  longdouble psi = source_pars.psi;
  longdouble phi0 = source_pars.phi0;
  longdouble Amp = source_pars.Amp;
  longdouble iota = source_pars.iota;

  longdouble om_0 = 2.*PI*fr;
   
  longdouble a1 = Amp*( (1.0 + cos(iota)*cos(iota))*cos(psi)*cos(phi0) - 2.0*cos(iota)*sin(psi)*sin(phi0) );
  longdouble a2 = Amp*( (1.0 + cos(iota)*cos(iota))*sin(psi)*cos(phi0) + 2.0*cos(iota)*cos(psi)*sin(phi0) );
  longdouble a3 = Amp*( (1.0 + cos(iota)*cos(iota))*cos(psi)*sin(phi0) + 2.0*cos(iota)*sin(psi)*cos(phi0) );
  longdouble a4 = Amp*( (1.0 + cos(iota)*cos(iota))*sin(psi)*sin(phi0) - 2.0*cos(iota)*cos(psi)*cos(phi0) );
   
  longdouble k[3] = {sin(theta_s)*cos(phi_s), sin(theta_s)*sin(phi_s), cos(theta_s)};
  longdouble n[3] = {sin(theta_a)*cos(phi_a), sin(theta_a)*sin(phi_a), cos(theta_a)};
  longdouble Lp = L_a*1.e3*pc_sec;
  longdouble Mc_sec = Mc*Msun;
  
  longdouble tau = Lp*(1. + k[0]*n[0] + k[1]*n[1] + k[2]*n[2]);
  longdouble om_orb = 0.5*om_0;
  longdouble om_p =  om_0*pow((1.0 + 256./5.*pow(Mc_sec,(5./3.))*pow(om_orb,(8./3.))*tau),(-3./8.)) ;
  longdouble phi_a_phase = -om_0*tau;

  struct geo_par geo;
  geo = ComputeFs(theta_s, phi_s, theta_a, phi_a);

  int i;
  longdouble phase_e,phase_p,sn,cs,sn_p,cs_p,h;
  longdouble denom_2 = (pow(om_p,(1./3.))*pow(om_0,(2./3.)));
  //  printf("\tCompute signal with %g\n",(double) om_0);
...
//#ifdef SINGLE
//double compute_likelihood(Cmatrix * phi,Cmatrix * FNF, Cvector * FNT,double tNt, double detN, int Nplsr, double ss, double detF)
//#else
//double compute_likelihood(Cmatrix * phi,Cmatrix * FNF, Cvector * FNT,double tNt, Cmatrix * Ninv, double detN, int Nplsr)
//#endif
...
//  w = new Cvector(psr->N_m);
  ...
  Cmatrix *Linv;
  Linv = new Cmatrix(psr->N_m,psr->N_m);

  Cmatrix *C;
  C = new Cmatrix(psr->N_m,psr->N_m);
...
//  Cmatrix * Ctemp;
//  Ctemp = new Cmatrix(psr->N_m,psr->N_m);
...
//	}
	...
	printf("tCt %s %d\t%g\n",pulsars[i].name,i,tCt);
      ...
      //need matric H^T C H
      Cmatrix * CH;
      CH = new Cmatrix(pulsars[i].N_m,2);
      Cmatrix * HCH;
      HCH = new Cmatrix(2,2);
...
//
//void initialize_FNF(Cvector * GT, Cmatrix *G_large, Cmatrix * GNG_large, struct mypulsar * pulsars, Cmatrix * F,Cmatrix *S, int Ntot, int Nplsr, int Ndim)
//{
...
//    }
//
  pulsars = (struct mypulsar *) malloc(MAX_PSR * sizeof(struct mypulsar));
