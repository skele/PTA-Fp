...
char distance_keys[HAVEDIST][12] = {"J0030+0451\0","J0610-2100\0","J0613-0200\0","J0751+1807\0","J0900-3144\0","J1012+5307\0","J1024-0719\0","J1455-3330\0","J1600-3053\0","J1640+2224\0","J1643-1224\0","J1713+0747\0","J1730-2304\0","J1738+0333\0","J1744-1134\0","J1804-2717\0","J1857+0943\0","J1909-3744\0","J1910+1256\0","J1918-0642\0","J1939+2134\0","J2010-1323\0","J2019+2425\0","J2124-3358\0","J2145-0750\0","J2317+1439\0","J2322+2057\0"} ;
...
{
  data = new double [n];
  std::memset(data,0.0,n*sizeof(double));
}

Cvector::Cvector(int *count, int n)
{
  data = new double [n];
...
public:
  ...
  Cmatrix(int *, int, int);
  ~Cmatrix();
  Cmatrix operator += (const Cvector*);
...
{
  data = new double [m*n];
  std::memset(data,0.0,n*m*sizeof(double));
}

Cmatrix::Cmatrix(int *count, int m, int n)
{
  data = new double [m*n];
  *count += m*n*sizeof(double);
  std::memset(data,0.0,n*m*sizeof(double));
}
...
Cmatrix Cmatrix::operator+=(const Cvector * B)
{
  int i,j;
  if ((m != B->n))
    {
...
struct mypulsar
{
  ...
  double dmA,dmgamma;
  double tNt;
  Cmatrix *G,*CWN,*GNGinv,*F,*H,*C,*Cinv,*L,*FNF;
  Cmatrix *GF,*GH,*sample;
  Cvector *toa,*res,*Gres,*phi_inv,*obsfreqs,*FNT;
};
...
extern "C" void dtrmv_(char *uplo,char *TA,char *diag,int *n,double *a,int *lda,double * b,int * incx);

//#ifndef CULA
extern "C" void dpptrf_(char *uplo, int *n, double *A, int *info);
extern "C" void dpotri_(char *uplo, int *n, double *A, int *lda, int *info);
extern "C" void dgemm_(char *trans, char * transb, int *m, int*n, int*k, double * alpha, double *A, int *lda, double *B, int * ldb, double *beta, double *C, int * ldc);
extern "C" void dsymm_(char *side, char *uplo, int *m, int *n, double *alpha, double *a, int *lda, double *b, int *ldb, double *beta, double *c, int *ldc);
...
void my_matrix_print_pretty(char * header, Cmatrix * a)
{
  int i,j;
  printf("%10s\n",header);
  for (i = 0; i < a->m; i++)
...
void my_matrix_print(char * header, Cmatrix * a)
{
  int i,j;
  for (i = 0; i < a->n; i++)
    {
...
void my_vector_mult(Cvector * a, Cvector *b, double * result)
{
  double temp = 0;
  int i;
  for (i = 0; i < a->n; i++)
...
int my_matrix_sub(Cmatrix * a, Cmatrix * b)
{
  int i,j;
  if ((a->m != b->m) || (a->n != b->n))
    {
...
void my_matrix_memcpy(Cmatrix * a, Cmatrix * b)
{
  memcpy(a->data,b->data,a->n*a->m*sizeof(double));
}

void my_vector_print(Cvector * vec)
{
  int i;
  for (i = 0; i < vec->n; i++)
    printf("%d\t%g\n",i,vec->data[i]);
...
int my_geev(Cmatrix *A, Cmatrix *evec, Cvector * eval)
{
  int lda;
  char jobvl = 'N';
...
int my_svd(Cmatrix * A, Cmatrix * U, Cvector *singular)
{
  int lda;
  lda = A->m;
  char jobu = 'A';
...
int my_dtrmv(int TransA, Cmatrix *A, Cvector * B)
{
  char uplo = 'L';
  char TA;
  int lda = A->m;
...
int my_dsymv(double alpha, Cmatrix * A, Cvector * B, double beta, Cvector * result)
{
  int incx,incy,n,lda;
  incx = 1; incy = 1;
  lda = A->m;
...
int  my_dgemv(int TransA,double alpha,Cmatrix * A,Cvector * B,double beta,Cvector * result)
{
  char TA;
...
int my_dsymm(int side, double alpha, const Cmatrix * A, const Cmatrix * B, double beta, Cmatrix * C)
{
  if (A->m != A->n)
    {
...
int my_dgemm(int TransA, int TransB, double alpha, const Cmatrix * A, const Cmatrix * B, double beta, Cmatrix * C)
{  
#ifdef CULA
  culaStatus s;
#endif
...
int get_inverse_lu(Cmatrix *m, Cmatrix *lu, int dim, double *det)
{
  char uplo;
  int N,i,j,lda;
  //  double *A;
...
int get_inverse_cholesky(Cmatrix *m, Cmatrix *cholesky, int dim)
{
  char uplo;
  int N,i,j,lda;
  //  double *A;
...
//void test_inversion(Cmatrix *a, Cmatrix *ainv)
//{
//  Cmatrix * prod;
...
//}
//
//void test_inversion_sym(Cmatrix *a, Cmatrix *ainv)
//{
//  Cmatrix * prod;
//  prod = my_matrix_alloc(a->m,a->m);
